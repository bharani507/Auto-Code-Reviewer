...
export const codeReview = async (bot, options, prompts) => {
    ...
    if (!files) {
        ...
        await commenter.comment(`Skipped: no files to review`, SUMMARIZE_TAG, 'replace');
        return;
    }
    ...
    if (filtered_files.length > options.max_files && options.max_files > 0) {
        ...
        await commenter.comment(`Skipped: too many files to review, can't handle it`, SUMMARIZE_TAG, 'replace');
        return;
    }
        ...
        const reviewPromises = files_to_review.map(async ([filename, file_content, file_diff, patches]) => openai_concurrency_limit(async () => {
            ...
            for (const [line, patch] of patches) {
                ...
                try {
                    ...
                    const all_chains = await commenter.get_conversation_chains_at_line(context.payload.pull_request.number, filename, line, COMMENT_REPLY_TAG);
